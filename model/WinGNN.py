...
from model.topo_layer import AttentionFusion, GatedFusion, ElementWiseMultiplicationFusion, ElementWiseAdditionFusion, ConcatenationFusion, BilinearFusion, FiLMFusion
from model.cnn_layer import MultiChannelCNN, FusionCNN

topo_dict = {'Attention': AttentionFusion, 'Gated': GatedFusion, 'Multiplication': ElementWiseMultiplicationFusion, 'Addition': ElementWiseAdditionFusion, 'Concatenation': ConcatenationFusion, 'Bilinear': BilinearFusion, 'FiLM': FiLMFusion}
cnn_dict = {'MultiChannel': MultiChannelCNN, 'Fusion': FusionCNN}
...
class Model(nn.Module):
    def __init__(self, in_features, out_features, hidden_dim, num_layers, dropout, cfg):
        super(Model, self).__init__()
        self.num_layers = num_layers
        self.dropout = dropout
        self.topo = cfg.topo.use_topo
        self.id = cfg.topo.id
        ...
        if self.topo != 'off' and i in cfg.topo.id:
            for i in range(num_layers):
                cnn = cnn_dict[cfg.topo.cnn_type](out_features, cfg)
                topo_layer = topo_dict[cfg.topo.use_topo](out_features, out_features, out_features, cfg)
                self.add_module('cnn{}'.format(i), cnn)
                self.add_module('topo{}'.format(i), topo_layer)
    ...
    def forward(self, g, x, fast_weights=None):
        ...
        for i in range(self.num_layers):
            ...
            if self.topo != 'off' and i in self.id:
                # 确保topo_feature在正确的设备上
                if hasattr(g, 'topo_feature'):
                    g.topo_feature = g.topo_feature.to(device)
                    
                cnn_start = 10 + i * 6
                cnn_end = cnn_start + 6
                cnn_fast_weights = fast_weights[cnn_start:cnn_end] if fast_weights else None
                
                cnn = getattr(self, f'cnn{i}')
                topo = getattr(self, f'topo{i}')
                cnn_feat = cnn(g.topo_feature, fast_weights=cnn_fast_weights)
                x = topo(x, cnn_feat)
